...
class Program
{
    ...
    {
        // Nie wiem czemu ale czasem mam problemy z debugowaniem, czasem dostaje wszystkie punkty a czasem 0, możliwe że coś się zacina
        ...
        points += Test(() => map1.HashCode("ABC") == Math.Abs(string.GetHashCode("ABC") % 101) && map2.HashCode(1290) == Math.Abs(1290.GetHashCode() % 101)
            ? 1
            : 0, 1);
        points += Test(() => map1.PutIfAbsent("ABC", 100) && map1.Contains("ABC") && !map1.PutIfAbsent("ABC", 100)
            ? 1
            : 0, 2);
        points += Test(() => map1.Replace("ABC", 200) && map1.Contains("ABC") && map1.Get("ABC") == 200
            ? 1
            : 0, 3);
        points += Test(() =>
        {
            return map1.Remove("ABC") && !map1.Contains("ABC") && map1.Get("ABC") == 0
                ? 1
                : 0;
...
public class HashMap<Key, Value> : IEnumerable<Pair<Key, Value>>
{
    ...
    { 
        ...
        {
            _values[HashCode(key)].Find(match: x => x.Key.Equals(key)).Value = value;
            return true;
        }
    }
    ...
    public bool Remove(Key key)
    {
        ...
        var pair = list.Find(x => x.Key.Equals(key));
        list.Remove(pair);
        _keys.Remove(key);
        return true;                                           
