...
namespace lab5_zadania
{
    ...
    public class StringMergeSort
    {
        ...
        private static string[] StringSortArray(string[] values, int left, int right)
        {
            if (left == right)
            {
                return new[] { values[left] };
            }
            if (left + 1 == right)
            {
                ...
                {
                    return new[] { values[left], values[right] };
                }
                else
                {
                    return new[] { values[right], values[left], };
                }
            ...
            var leftArr = StringSortArray(values, left, mid);
            var rightArr = StringSortArray(values, mid + 1, right);
        ...
        private static string[] StringMerge(string[] arr1, string[] arr2)
        {
            ...
            for (int i = 0, j1 = 0, j2 = 0; i < result.Length; i++)
            {
                if (j1 < arr1.Length && j2 < arr2.Length)
                {
                    ...
                    if (arr1[j1].Length < arr2[j2].Length)
                    {
                        result[i] = arr1[j1++];
                    }
                    else
